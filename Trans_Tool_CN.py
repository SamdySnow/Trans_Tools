...
query = input('请输入需要查询的单词或句子...\n')
#query = 'happy'

with open(r'python\Trans Tool\sign_func.js', 'r', encoding='utf-8') as f:
...
if(response.status_code == 200):
    print("状态代码: \033[32m●\033[0m 200 服务器主机 %s\n"%colorFormat('在线','green'))
#print(response.json())
...
def dataAna_trans_result(dict):
    ...
    print('请求查询        : %s'%colorFormat(src,'green'))
    print('翻译结果        : %s'%colorFormat(dst,'green'))

    phonetic_list = trans_result['phonetic']
    s = ''
    for i in phonetic_list:
        s = s + i['trg_str'] + ' '
    print('拼音            : %s'%colorFormat(s,'green'))

def print_word_exchange(exchange_dict):

    print(colorFormat('时态与比较/最高级变换:','cyan'))
    ...
    if exchange_dict.__contains__('word_third'):
        ...
        for i in s[0:-1]:
            word += i + ','
        word += s[-1]
        print('  --> %s: %s' % (colorFormat('第三人称单数' ,'blue'),
    ...
    if exchange_dict.__contains__('word_ing'):
        ...
        for i in s[0:-1]:
            word += i + ','
        word += s[-1]
        print('  --> %s: %s' % (colorFormat('现在进行时  ', 'blue'),
                                colorFormat(word, 'green'),))
    ...
    if exchange_dict.__contains__('word_past'):
        ...
        for i in s[0:-1]:
            word += i + ','
        word += s[-1]
        print('  --> %s: %s' % (colorFormat('一般过去时  ', 'blue'),
                                colorFormat(word, 'green'),))
    ...
    if exchange_dict.__contains__('word_done'):
        ...
        for i in s[0:-1]:
            word += i + ','
        word += s[-1]
        print('  --> %s: %s' % (colorFormat('过去完成时  ', 'blue'),
                                colorFormat(word, 'green'),))
    ...
    if exchange_dict.__contains__('word_pl'):
        ...
        for i in s[0:-1]:
            word += i + ','
        word += s[-1]
        print('  --> %s: %s' % (colorFormat('复数形式    ', 'blue'),
                                colorFormat(word, 'green'),))
    ...
    if exchange_dict.__contains__('word_er'):
        ...
        for i in s[0:-1]:
            word += i + ','
        word += s[-1]
        print('  --> %s: %s' % (colorFormat('比较级      ', 'blue'),
                                colorFormat(word, 'green'),))
    ...
    if exchange_dict.__contains__('word_est'):
        ...
        for i in s[0:-1]:
            word += i + ','
        word += s[-1]
        print('  --> %s: %s' % (colorFormat('最高级      ', 'blue'),
                                colorFormat(word, 'green'),))

def printUsecase(usecase_dict):

    if usecase_dict.__contains__('collocation'):
        print(colorFormat('单词用法:', 'cyan'))
        print('  --> %s'%colorFormat('与以下词性连用:','cyan'))
...
def printSanyms(sanyms_list):
        print(colorFormat('同义词与反义词:','cyan'))

        for sanyms_dict in sanyms_list:
            if sanyms_dict['type'] == 'synonym':
                print('  --> %s:'%colorFormat('同义词','yellow'))
            ...
            if sanyms_dict['type'] == 'antonym':
                print('  --> %s:' % colorFormat('反义词', 'yellow'))
                data_list = sanyms_dict['data']
...
def dataAna_dict_result(dict):
    ...
    print(colorFormat('单词释义:','cyan'))
    print('  --> %s'%colorFormat(word_means,'green'))

    parts = symbols['parts']

    print(colorFormat('单词详细释义:','cyan'))
    ...
    for item_dict in item_list:
        ...
        for tr_group in tr_group_list:
            ...
            print('    --> %s: %s'%(colorFormat('相似词语','yellow'),colorFormat(similar_word,'green')))

            for ex_i in tr_group['example']:
                example += ex_i + '; '
            
            print('    --> %s: %s'%(colorFormat('例句','yellow'),colorFormat(example,'green')))
...
fp = open(r'python\Spider Study\1.Ajax\test.json','w',encoding='utf-8')
